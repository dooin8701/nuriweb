const DATA_LIST =[
    [//뉴스
        {list_no: 1, item_name: 'OO씨와 OO씨의 열애설', user_name: 'user1', day: '2023-12-20', item_idx: '가수 OO씨와 배우 OO씨의 열애설에 대한 소문이 돌고있는 가운데 두분이 열애 사실을 시인했다는 소식입니다.'},
...
const LIST_TITLE_INFO = ['[NEWS]', '[INFO]', '[USER]', '[COMMU]', '[공지사항]'];
...
function make_info_page(cate, item){
    $('.cate_title').text(LIST_TITLE_INFO[cate])
    $('.item_title').text(DATA_LIST[cate][item].item_name)
    $('.user_idx').text(DATA_LIST[cate][item].user_name)
    $('.day_idx').text(DATA_LIST[cate][item].day)
    $('.idx_main').text(DATA_LIST[cate][item].item_idx)
}
